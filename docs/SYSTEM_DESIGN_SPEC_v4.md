# Echo UGC — 系统设计规范 v4.0 (System Design Specification)

> **文档定位**: Schema First + Interface Contract + Domain Model Design + Five-Graph Model Editor
> **版本**: v4.0 — 在 v3.0 基础上新增 §18 6维约束体系接口注册区
> **日期**: 2026-08-03
> **变更**: 从 v3.0 升级，新增 §18 6维约束体系接口注册区（枚举定义/权重矩阵/数据模型/接口签名/API端点/数据流图/命名碰撞消解）
...
11. [五图模型映射](#11-五图模型映射)
12. [命名规范](#12-命名规范)
13. [LLM Prompt 设计规范](#13-llm-prompt-设计规范)
14. [错误处理与边界条件](#14-错误处理与边界条件)
15. [测试策略](#15-测试策略)
16. [部署与运维](#16-部署与运维)
17. [分阶段实施接口注册区](#17-分阶段实施接口注册区)
18. [6维约束体系接口注册区](#18-6维约束体系接口注册区)
13. [依赖注入架构](#13-依赖注入架构)
14. [实施路线图](#14-实施路线图)
...
class Asset(BaseModel):
    ...
    # V3 新增 — 追加到末尾，向后兼容
    classification: AssetClassification = AssetClassification.ENVIRONMENT  # 由来源图决定
...
class StoryCondition(BaseModel):
    ...
    requirement: str | dict[str, Any] # 禁止裸 any
...
class EventAction(BaseModel):
    """事件动作。"""
    type: str
    parameters: dict[str, Any] = Field(default_factory=dict)  # 类型明确
...
class ConstraintNode(BaseModel):
    ...
    type: ConstraintType
    rule: str                           # 规则描述文本
    rule_config: dict[str, Any] | None = None  # 规则配置可选
    priority: int = 0                   # 0-100，数值越大优先级越高
    applicable_types: list[str] = Field(default_factory=list)  # 适用的资产类型或分类
...
class StoryGraphUpdate(BaseModel):
    ...
    edges: list[StoryEdge] | None = None
```
...
class CultureNodeCreate(BaseModel):
    ...
    values: list[str] = Field(default_factory=list)
    aesthetic_principles: list[str] = Field(default_factory=list)
    child_nodes: list[CultureNodeCreate] = Field(default_factory=list)
```
...
## 18. 6维约束体系接口注册区 (V4 新增)

> **本章为 V4 新增**：定义6维约束体系的完整接口规范，包括枚举定义、权重矩阵配置、数据模型、服务层接口、API端点契约和数据流图。
> **权威源声明**: 实验代码 `backend/experiments/layered_weight_experiment.py` 为接口定义参考（但不作为实现代码）。
> **设计原则**: Schema First + Interface Contract — 本节仅定义接口签名和数据结构，不涉及实现细节。

### 18.1 概述

**6维约束体系**是"种子→6维→五图"生成管道的上游概念。该体系为不同创作层级（世界观/区域文化/场景/战役/NPC/资产）提供语义化的约束分类。

#### 核心概念

- **6个约束维度**：按语义分类，每类约束代表对象在特定方面的限制或规则
  - RED (内容红线) — 禁止清单
  - LAW (物理法则) — 世界规则
  - ACT (行为规则) — 判定逻辑
  - NAR (叙事约束) — 文风风格
...
- **6个创作层级**：从宏观到微观的创作层级
  - world (世界观) → region (区域文化) → scene (场景) → campaign (战役) → npc (NPC) → asset (资产)

- **权重矩阵**：每个层级对各维度分配不同的权重（总和100），指导AI生成时的注意力分配

#### 与五图模型的关系

6维约束体系与五图模型中的 **ConstraintTree** 是不同层次的概念：

| 对比维度 | 6维约束体系 | ConstraintTree (五图) |
|---------|------------|----------------------|
| **用途** | 上游概念生成，为种子赋予语义约束 | 下游资产生成规则，判定候选是否合规 |
| **层级关系** | "种子→6维→五图" pipeline 的一部分 | 五图模型中的约束节点树 |
| **约束类型** | ConstraintDimension (语义分类：RED/LAW/ACT/NAR/WST/SOC) | ConstraintType (强制力：HARD/SOFT) |
| **数据模型** | DimensionResultSet (6个维度输出) | ConstraintTree (约束节点列表) |
| **共存关系** | 可与 ConstraintType 共存，一个约束节点可同时拥有维度和强制力属性 | 独立的约束树结构 |

### 18.2 枚举定义

#### ConstraintDimension (约束维度枚举)

| 枚举值 | 名称 | 描述 | data_essence |
|-------|------|------|--------------|
| RED | "RED" | 内容红线 | 这个对象不能包含什么（禁止清单） | 禁止的内容条目 |
| LAW | "LAW" | 物理法则 | 这个对象服从什么世界规则（重力/魔法体系/科技水平） | 规则函数 |
| ACT | "ACT" | 行为规则 | 作用于这个对象的行为怎么判定（技能检定/社交反馈/伤害计算） | 判定逻辑 |
| NAR | "NAR" | 叙事约束 | 描述这个对象时遵循什么风格（文风/词汇/氛围/基调） | 风格指令文本 |
| WST | "WST" | 世界状态 | 这个对象携带什么动态状态效果（天气/环境修正/buff/debuff） | 修正值/状态效果 |
| SOC | "SOC" | 社交生态 | 这个对象和谁有关系、什么关系（阵营/声望/亲缘/敌对） | 关系边 |

**注**：枚举值逐字列出，不允许添加或修改。

#### CreationLayer (创作层级枚举)
...
权重矩阵定义每个创作层级对各约束维度的关注度分配（每行和为100）。

#### YAML 配置文件

**文件路径**: `data/weight_matrix.yaml`
...
| Layer | RED | LAW | ACT | NAR | WST | SOC | Sum |
|-------|-----|-----|-----|-----|-----|-----|-----|
| world | 20 | 30 | 5 | 35 | 5 | 5 | 100 |
| region | 10 | 10 | 10 | 20 | 5 | 45 | 100 |
| scene | 10 | 25 | 5 | 10 | 40 | 10 | 100 |
| campaign | 10 | 5 | 10 | 40 | 5 | 30 | 100 |
| npc | 5 | 10 | 25 | 20 | 10 | 30 | 100 |
| asset | 10 | 30 | 30 | 10 | 10 | 10 | 100 |

#### 验证规则 (8种异常场景)

权重矩阵加载时需验证以下8种异常情况，异常时抛出 `WeightMatrixError(row, detail)`：

| 异常场景 | 检测条件 | 异常抛出 (WeightMatrixError) |
|---------|---------|------------------------------|
| 1. 文件缺失 | 文件不存在 | `row=None, detail="file not found: <path>"` |
| 2. YAML 语法错误 | YAML 解析失败 | `row=None, detail="yaml parse error: <parse_error>"` |
| 3. 缺层 (不是6层) | 缺少任一层级键 | `row="<missing_layer>", detail="missing layer in weight matrix"` |
| 4. 缺维度 (某行不是6维) | 某层级键缺少任一维度键 | `row="<layer>", detail="missing dimension: <dim>"` |
| 5. 行和≠100 | 某层级所有维度权重和不等于100 | `row="<layer>", detail="row sum is <sum>, expected 100"` |
| 6. 负值 | 某维度权重 < 0 | `row="<layer>", detail="negative weight for <dim>: <value>"` |
| 7. 浮点值 (非整数) | 某维度权重不是整数 | `row="<layer>", detail="non-integer weight for <dim>: <value>"` |
| 8. 多余键 | 配置中存在非预期键 | `row="<layer>", detail="unexpected key: <key>"` |

### 18.4 6维输出数据模型

#### 维度输出子模型

每个约束维度对应一个 Pydantic 输出模型，定义该维度的数据结构：

以下6个子模型统称 **DimensionOutput** 系列，每个维度对应一个 typed Pydantic model（而非裸 `dict[str, Any]`）：

| 模型 | 字段 | 类型 | 默认值 | 说明 |
|-----|------|------|-------|------|
| **RedOutput** | forbidden | `list[str]` | `[]` | 禁止的内容条目列表 |
| | note | `str` | `""` | 说明文本（如果没有禁忌可留空） |
| **LawOutput** | rules | `list[str]` | `[]` | 规则描述列表 |
| | mechanism | `str` | `""` | 核心运行机制描述 |
| **ActOutput** | actions | `list[dict[str, Any]]` | `[]` | 每个action有 `trigger/check/success/failure` 字段 |
| **NarOutput** | style | `str` | `""` | 文风/氛围描述 |
| | keywords | `list[str]` | `[]` | 关键词列表 |
| | tone | `str` | `""` | 基调（如：压抑/壮丽/诡异） |
| **WstOutput** | effects | `list[dict[str, Any]]` | `[]` | 每个effect有 `name/type/magnitude` 字段 |
| **SocOutput** | relations | `list[dict[str, Any]]` | `[]` | 每个relation有 `target/type/value` 字段 |

#### 容器模型

`DimensionResultSet` 是包含所有6个维度输出的容器模型：
...
| 文件路径 | 操作 | 主要类/函数 |
|---------|------|-------------|
| `data/weight_matrix.yaml` | 新建 | YAML配置（18.3节定义的6×6权重矩阵） |
| `backend/app/models/dimension.py` | 新建 | `ConstraintDimension`, `CreationLayer`, `DimensionInfo`, `WeightMatrix`, `WeightMatrixEntry`, `DimensionResultSet`, `RedOutput`, `LawOutput`, `ActOutput`, `NarOutput`, `WstOutput`, `SocOutput`, `WeightMatrixError` |
| `backend/app/services/dimension_generator.py` | 新建 | `WeightMatrixLoader`, `DimensionPromptBuilder`, `DimensionGenerator`, `DimensionParseError` |
| `backend/app/api/constraints_routes.py` | 新建 | `constraints_router`, `ConstraintGenerateRequest`, `ConstraintGenerateResponse` |
| `backend/app/config/paths.py` | 扩展 | 新增 `WEIGHT_MATRIX_PATH` 常量 |
| `backend/app/api/deps.py` | 扩展 | 新增 `get_dimension_generator()` 依赖 |
| `backend/app/main.py` | 扩展 | 注册 `constraints_router` |
...
class WeightMatrixEntry(BaseModel):
    """权重矩阵条目 — dict[ConstraintDimension, int] 的类型化包装。
    ...
    def get_weight(self, dim: ConstraintDimension) -> int:
        """获取指定维度的权重。"""
        return getattr(self, dim.value)


class WeightMatrix(BaseModel):
    """权重矩阵 — dict[CreationLayer, WeightMatrixEntry]。"""
...
class WeightMatrixLoader:
    ...
    def load(self) -> WeightMatrix:
        ...
        Raises:
            WeightMatrixError: 8种验证失败场景（见18.3节）
        """
        ...
...
class DimensionPromptBuilder:
    ...
    def build_user_prompt(
        ...
        """构建 user prompt，包含种子输入和描述。
        
        Args:
            seed_input: 种子输入（必需）
            seed_description: 种子描述（可选）
...
class DimensionGenerator:
    ...
    def __init__(
        ...
        Args:
            ...
            matrix_loader: 权重矩阵加载器（None则使用默认）
        """
        ...
    ...
    async def generate(
        ...
        seed_input: str,
        seed_description: str = "",
        layer: CreationLayer = CreationLayer.ASSET,
        intent: str = "",
    ) -> DimensionResultSet:
        """生成6维约束。
        ...
        Returns:
            DimensionResultSet (6个维度的输出)
            
        Raises:
            DimensionParseError: LLM输出验证失败
            WeightMatrixError: 权重矩阵配置错误
        """
        ...
...
class DimensionParseError(Exception):
    """LLM输出解析错误。"""
    original: dict[str, Any]  # 原始LLM输出
    errors: list[Any]         # Pydantic验证错误列表
```

#### constraints_routes.py — API路由
...
from app.models.dimension import CreationLayer, DimensionResultSet, DimensionParseError, WeightMatrixError
...
class ConstraintGenerateRequest(BaseModel):
    """约束生成请求。"""
    seed_input: str = Field(..., min_length=1, description="种子输入（必需）")
    seed_description: str = Field("", description="种子描述（可选）")
    layer: CreationLayer = Field(
        default=CreationLayer.ASSET,
        description="创作层级",
    )
...
async def generate_constraints(
    ...
    Args:
        request: 生成请求
        generator: 约束生成器（依赖注入）
        
    Returns:
        ConstraintGenerateResponse
        
    Raises:
        HTTPException(422): 无效的layer / 缺少seed_input / 空seed_input
        HTTPException(502): DimensionParseError (LLM输出验证失败)
        HTTPException(503): WeightMatrixError (权重矩阵配置问题)
    """
    ...
```
...
```json
{
  "seed_input": "古代水晶祭坛",       // required, min_length=1
  "seed_description": "由整块水晶雕刻而成，散发着微弱的魔法光芒", // optional, default ""
  "layer": "asset",                  // required, enum: world/region/scene/campaign/npc/asset
  "intent": "在资产层定义一个具有神秘属性的祭坛对象"  // optional, default ""
}
```
...
```json
{
  ...
  "dimensions": {
    "RED": {
      "forbidden": ["不能包含现代科技元素", "不能具有自我意识"],
      "note": "这是一个古代魔法物品，保持传统神秘感"
    },
    "LAW": {
      "rules": [
        "遵循世界魔法体系：需要通过魔力共鸣激活",
      ...
      "actions": [
        {
          ...
          "check": "魔力检定 DC 15",
          "success": "激活祭坛，显示预言影像",
          "failure": "轻微魔力反噬，受到1点精神伤害"
        }
      ]
    },
    "NAR": {
      "style": "庄重神秘，带有古老传说的色彩",
      "keywords": ["符文", "预言", "共鸣", "古代", "魔力"],
      "tone": "压抑而神圣"
    },
    "WST": {
      "effects": [
...
| 状态码 | 场景 | 响应体 |
|-------|------|--------|
| 422 | Invalid layer / missing seed_input / empty seed_input | `{"detail": "Invalid layer: xxx"}` |
| 502 | DimensionParseError (LLM输出验证失败) | `{"detail": "DimensionParseError: ...", "original": {...}, "errors": [...]}` |
| 503 | WeightMatrixError (权重矩阵配置问题) | `{"detail": "WeightMatrixError: ...", "row": "xxx", "detail": "..."}` |

### 18.8 数据流图

```
WeightMatrix(YAML file)
    │
    ▼
WeightMatrixLoader.load()
    │ (validates 8 scenarios: 文件缺失/YAML语法/缺层/缺维度/行和≠100/负值/浮点/多余键)
    ▼
WeightMatrix (Pydantic model)
    │
    ├──► WeightMatrixLoader.get_weights(layer) ──► WeightMatrixEntry
    │                                                    │
    │                                                    ▼
    │                                    DimensionPromptBuilder.build_system_prompt()
    │                                                    │
    │                                                    ▼
    │                                              system_prompt
    │                                                    │
    ├──► seed_input, seed_description ──► build_user_prompt() ──► user_prompt
    │                                                                   │
    ▼                                                                   ▼
DimensionGenerator.generate() ◄───────────────────────── messages = [{system}, {user}]
    │
    ▼
LLMProvider.chat_json(messages)
    │
...
raw_dict: dict[str, Any]
    │
    ▼
DimensionResultSet.model_validate(raw_dict)
    │ (Pydantic validation — 6个维度模型验证)
    ▼
DimensionResultSet (6 typed dimension outputs: RED/LAW/ACT/NAR/WST/SOC)
    │
    ▼
ConstraintGenerateResponse (layer + weights + dimensions + elapsed_seconds)
```

### 18.9 命名碰撞消解声明

**声明：ConstraintDimension ≠ ConstraintType**

系统中存在两个容易混淆的枚举类型，它们代表不同的概念分类：

#### ConstraintDimension (新增，6维约束体系)

- **文件位置**: `backend/app/models/dimension.py`
- **枚举值**: `RED / LAW / ACT / NAR / WST / SOC`
- **语义**: 约束的内容类别（语义分类）—— 这个约束是关于哪方面的？
- **用途**: 6维约束体系，标记每类约束是什么方面的（内容/物理/行为/叙事/状态/社交）
- **层级**: 上游概念生成，"种子→6维→五图" pipeline

#### ConstraintType (已有，五图模型)

- **文件位置**: `backend/app/models/constraint.py`
- **枚举值**: `HARD / SOFT`
- **语义**: 约束的强制力级别 —— 这个约束有多强制？
- **用途**: 五图模型 ConstraintTree，标记约束是硬性（违反则失败）还是软性（影响评分）
- **层级**: 下游资产生成规则，候选过滤与评分

#### 共存关系

两者共存于同一代码库，**不互相替换**。一个约束节点可以同时拥有：
- `ConstraintDimension` (内容类别) —— 标记这个约束是 RED/LAW/ACT/NAR/WST/SOC
- `ConstraintType` (强制力级别) —— 标记这个约束是 HARD 还是 SOFT

#### 示例

```python
# 示例：一个既是"物理法则"维度又是"硬约束"的规则
constraint = ConstraintNode(
    type=ConstraintType.HARD,           # 硬约束
    dimension=ConstraintDimension.LAW,   # 物理法则维度
    rule="这个世界遵循重力规则",
    rule_config={"gravity": 9.8}
)
```

**重要**: 不要混淆或合并这两个枚举，它们服务于不同的抽象层次。
...
**文档状态**: ✅ 已完成 v4.0 §18 6维约束体系接口注册区
**下一步**: 实施6维约束生成服务（Phase 6）
**负责人**: 开发团队
